    ...
    <div class="card">
        <div>
            <div class="post-header">
                ...
                <MenuDropdown :icon="`bi bi-three-dots`" v-if="isMyPost">
                    <template #options>
                        <span class="tab popup-tab" @click="openUpdatePost"><i class="bi bi-pencil"></i><span
                                class="tab-text">Update</span></span>
                        <span class="tab popup-tab" @click="handleDeletePost"><i class="bi bi-trash3"></i><span
                                class="tab-text">Delete</span></span>
                    </template>
                </MenuDropdown>
                <PostCreate v-if="showUpdatePost" :data-post="props.post" @updatePost="updatePost"
        ...
        <div @click="openDetailPost">
            ...
            <div class="btn-group tab-list-col">
                <span @click.stop="likePost" class="tab btn btn-action"
                    :class="{ 'btn-primary': isLiked, 'btn--light': !isLiked }"><i class="bi bi-hand-thumbs-up"></i> <span
                        class="tab-text">Like {{ postLikesLength }}</span></span>
                <span class="tab btn-action"><i class="bi bi-chat"></i> <span class="tab-text">Comment {{
                        postCommentsLength
                        }}</span></span>
                <span class="tab btn-action"><i class="bi bi-share"></i> <span class="tab-text">Share</span></span>
            </div>
            <PostDetail @click.stop="" v-if="showDetailPost" :post="post" @deletePost="handleDeletePost"
                @close="closeDetailPost"></PostDetail>
        </div>
    </div>
</template>
